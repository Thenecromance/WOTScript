...
class NyVehiclesViewPresenter(HistorySubModelPresenter):
    ...
    def createToolTipContent(self, event, ctID):
        if ctID == R.views.lobby.new_year.tooltips.new_year_vehicle_bonus.NewYearVehiclesBonus():
            return NewYearVehiclesBonusTooltip()
        return super(NyVehiclesViewPresenter, self).createToolTipContent(event, ctID)
