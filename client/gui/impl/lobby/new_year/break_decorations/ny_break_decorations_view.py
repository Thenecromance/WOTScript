...
class NyBreakDecorationsView(HistorySubModelPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.new_year.tooltips.NyDecorationStateTooltip():
            return NyDecorationStateTooltip(event.getArgument('atmosphereBonus'))
        return super(NyBreakDecorationsView, self).createToolTipContent(event, contentID)

    def createPopOverContent(self, event):
        if event.contentID == R.views.lobby.new_year.popovers.NyBreakFilterPopover():
            return NyBreakFilterPopoverView(self.__selectedToyFilters)
        return super(NyBreakDecorationsView, self).createPopOverContent(event)
