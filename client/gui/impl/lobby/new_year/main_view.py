...
from uilogging.ny.loggers import NyMainViewFlowLogger
...
_STAGE_ENABLED_TAB = (NyWidgetTopMenu.GLADE, NyWidgetTopMenu.CHALLENGE)

class MainView(ViewImpl, GiftEventHubWatcher):
    ...
    def __switchSubView(self, ctx):
        ...
        if subModelInfo.canBeLoaded is not None and not subModelInfo.canBeLoaded():
            ...
            if subModelInfo.ID == MainViews.INFO:
                prevViewAlias = ctx.kwargs.get('previousViewAlias')
                if prevViewAlias is None:
                    return
    ...
    def __checkPresenters(self):
        if self.isGiftEventEnabled():
            ...
            if subModelInfo.ID == MainViews.INFO:
                ...
                self.__backCallback = partial(NewYearNavigation.switchToView, ViewAliases.GLADE_VIEW)
                with self.viewModel.transaction() as tx:
                    tx.backButton.setIsVisible(True)
                    tx.backButton.setCaption(backport.text(R.strings.ny.backButton.label()))
                    tx.backButton.setGoTo(backport.text(R.strings.ny.backButton.NyGladeView()))
            return
...
class _PresentersMap(object):
    ...
    def __makeLoadersMap(self):
        return {NyWidgetTopMenu.GLADE: partial(self.__makeSubModel, MainViews.GLADE, self.__loadGlade),
         ...
         NyWidgetTopMenu.VEHICLES: partial(self.__makeSubModel, MainViews.VEHICLES, self.__loadVehicles, lambda : not self.__mainView.isGiftEventDisabled()),
         ...
         NyWidgetTopMenu.GIFT_SYSTEM: partial(self.__makeSubModel, MainViews.POST, self.__loadPost, self.__mainView.isGiftEventEnabled)}
    ...
    def __loadVehicles(self):
        from gui.impl.lobby.new_year.vehicles.ny_vehicles_view import NyVehiclesViewPresenter
        return NyVehiclesViewPresenter(self.__mainView.viewModel.vehiclesModel, self.__mainView)
    ...
    def __loadPost(self):
        from gui.impl.lobby.new_year.gift_system.ny_gift_system_view import NyGiftSystemView
        return NyGiftSystemView(self.__mainView.viewModel.giftSystemModel, self.__mainView)
