...
class RandomModeSelectorItem(ModeSelectorLegacyItem):
    ...
    def __getBattlePassName(self):
        ...
        if self.__battlePassController.isActive() or self.__battlePassController.isPaused():
            return backport.text(R.strings.mode_selector.event.battlePass.name(), seasonNum=seasonNum)
        return ''