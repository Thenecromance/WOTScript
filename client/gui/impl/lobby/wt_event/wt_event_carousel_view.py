...
class WTEventCarouselView(ViewImpl, IGlobalListener):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.wt_event.tooltips.WtEventAmmunitionTooltipView():
            return WtEventAmmunitionTooltipView(intCD=event.getArgument('id'))
...
def _fillEquipment(array, item):
    ...
    for eq in item.consumables.layout:
        ...
        if 'repairkit' in eq.tags or 'medkit' in eq.tags:
            ...
            abilities.append(eq)
        elif 'eventPassiveItem' in eq.tags:
            passives.append(eq)
