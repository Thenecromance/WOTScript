...
class EventBattlePage(ClassicPage, ISpawnListener):
    ...
    def _spawnViewAlias(self):
        if self._spawnType == SpawnType.TELEPORT:
            return BATTLE_VIEW_ALIASES.EVENT_BOSS_TELEPORT
        return BATTLE_VIEW_ALIASES.EVENT_HUNTER_RESPAWN

    @property
    def _spawnVisibleComponents(self):
        if self._spawnType == SpawnType.TELEPORT:
            return {BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL, BATTLE_VIEW_ALIASES.BATTLE_TIMER}
        return {BATTLE_VIEW_ALIASES.PLAYERS_PANEL_EVENT, BATTLE_VIEW_ALIASES.BATTLE_TIMER, BATTLE_VIEW_ALIASES.BATTLE_MESSENGER}
