...
class MapsTrainingBattleGoals(BattleHintComponent, MapsTrainingGoalsMeta, IArenaVehiclesController):
    ...
    def _getSoundNotification(self, hint, data):
        if hint.soundNotificationNewbie is not None and not self.settingsCore.serverSettings.getOnceOnlyHintsSetting(OnceOnlyHints.MAPS_TRAINING_NEWBIE_HINT, default=False):
            return hint.soundNotificationNewbie
        else:
