...
class BattleRoyaleTimersPanel(TimersPanel):
    ...
    def __getDestroyTimerTypeIDByCode(self, value):
        if value.code == VEHICLE_MISC_STATUS.VEHICLE_IS_OVERTURNED and getTimerViewTypeID(value.level) == BATTLE_NOTIFICATIONS_TIMER_TYPES.WARNING_VIEW:
            return BATTLE_NOTIFICATIONS_TIMER_TYPES.HALF_OVERTURNED
        return self._mapping.getTimerTypeIDByMiscCode(value.code)
