# Embedded file name: scripts/client/gui/Scaleform/daapi/view/battle/battle_royale/status_notifications/replay_components.py
import logging
import BattleReplay
...
class _ReplaySnapshotHandler(TimeSnapshotHandler):
    ...
    def __onTickerTick(self, timeLeft):
        ...
        self._updateHandler(currTime=currTime, totalTime=self.__totalTime, isVisible=True, isUpdateRequired=True)

    def __onTickerFinished(self):
        self.__destroyTicker()
        self._updateHandler(currTime=0, totalTime=0, isVisible=False, isUpdateRequired=True)
