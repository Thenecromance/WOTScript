...
def countSkinAsNew(item):
    if PersonalCase.crewSkinsHAConfig.isHistoricallyAccurate():
        return item.getHistorical()
    return True
...
class PersonalCase(PersonalCaseMeta, IGlobalListener):
    ...
    def __getTankmanByInvID(self, inventoryID):
        if inventoryID == self.tmanInvID:
            return self._tankman
        return self.itemsCache.items.getTankman(inventoryID)
