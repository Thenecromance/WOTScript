...
class RestrictedVehicleClassDataProvider(object):
    ...
    def getCurrentVheiclesOfClassCount(self):
        ...
        if unit is None:
            ...
            return sum((slot.player is not None and slot.player.isReady and slot.vehicle is not None and slot.vehicle.vehClassIdx == VEHICLE_CLASS_INDICES[self._VEHICLE_CLASS_NAME] for slot in self.__unitEntity.getSlotsIterator(unitMgrId, unit)))
