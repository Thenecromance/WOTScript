...
class VehicleParams(_ParameterBase):
    ...
    def wheeledSpeedModeSpeed(self):
        if self.__hasWheeledSwitchMode():
            return self.__speedLimits(self._itemDescr.siegeVehicleDescr.physics['speedLimits'], ('forwardMaxSpeedKMHTerm', 'backwardMaxSpeedKMHTerm'))
        else:
            return None
    ...
    def chargeTime(self):
        if self.__hasDualGun():
            return (float(self._itemDescr.gun.dualGun.chargeTime), self._itemDescr.gun.dualGun.reloadLockTime)
        else:
            return None
    ...
    def aimingTime(self):
        ...
        if self._itemDescr.hasTurboshaftEngine:
            return (aimingTimeVal, items_utils.getGunAimingTime(self._itemDescr.siegeVehicleDescr, self.__factors))
...
class GunParams(WeightedParam):
    ...
    def reloadTime(self):
        if self.getReloadingType() in (GUN_CAN_BE_AUTO_RELOAD, GUN_AUTO_RELOAD):
            return None
        elif self.getReloadingType() in (GUN_CAN_BE_DUAL_GUN, GUN_DUAL_GUN):
            return None
        else:
