...
class TankmanRecruitAndEquip(Processor):
    ...
    def _successHandler(self, code, ctx = None):
        ...
        if tmanCost:
            return makeI18nSuccess(sysMsgKey='{}/financial_success'.format(prefix), auxData=ctx, price=formatPrice(tmanCost, useStyle=True), type=sysMsgType)
        return makeI18nSuccess(sysMsgKey='{}/success'.format(prefix), auxData=ctx, type=sysMsgType)
...
class TankmanRetraining(ItemProcessor):
    ...
    def _successHandler(self, code, ctx = None):
        ...
        if self.tmanCost:
            return makeI18nSuccess(sysMsgKey='retraining_tankman/financial_success', auxData=ctx, type=sysMsgType, price=formatPrice(self.tmanCost))
        return makeI18nSuccess(sysMsgKey='retraining_tankman/success', auxData=ctx, type=sysMsgType)
...
class TankmanCrewRetraining(Processor):
    ...
    def _successHandler(self, code, ctx = None):
        ...
        if crewRetrainingCost:
            return makeI18nSuccess(sysMsgKey='{}/financial_success'.format(messagePrefix), auxData=ctx, type=sysMsgType, price=formatPrice(crewRetrainingCost))
        return makeI18nSuccess('{}/success'.format(messagePrefix), type=sysMsgType, auxData=ctx)
