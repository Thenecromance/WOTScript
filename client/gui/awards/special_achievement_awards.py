...
class CrewSkinAward(ExplosionBackAward):
    ...
    def getOkButtonText(self):
        if self._getFirstEnableTankman() is not None:
            return backport.text(R.strings.crew_skins.feature.poster.confirm())
        else:
            return backport.text(R.strings.menu.awardWindow.okButton())
