...
def _getHangarPath(isPremium, isPremIGR):
    ...
    if isPremIGR:
        return _HANGAR_CFGS[_getDefaultHangarPath(False)][_IGR_HANGAR_PATH_KEY]
    return _getDefaultHangarPath(isPremium)
...
def _getHangarVisibilityMask(isPremium):
    ...
    return SPACE_FULL_VISIBILITY_MASK
    ...
    def __onEventNotificationsChanged(self, diff):
        ...
        for notification in diff['removed']:
            if notification['type'] == SERVER_CMD_CHANGE_HANGAR:
                ...
                if not isPremium:
                    hasChanged = True
            elif notification['type'] == SERVER_CMD_CHANGE_HANGAR_PREM:
