...
_QUESTS_WITH_AWARDS = frozenset(['2_2',
 '2_4',
 '3_1',
 '3_2'])
...
class LinkedSetController(ILinkedSetController):
    ...
    def getMaxMissionID(self):
        ...
        if quests:
            return max((getLinkedSetMissionIDFromQuest(quest) for quest in quests.itervalues()))
        return -1
    ...
    def getCompletedLinkedSetQuests(self, filterFunc = None):
        ...
        def userFilterFunc(q):
            if self.isFinalQuest(q) and q.isAvailable().isValid:
                return filterFunc(q)
            return q.isCompleted() and filterFunc(q)
