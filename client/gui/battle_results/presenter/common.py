...
def getCrystalsRecords(reusable):
    ...
    if not crystalsRecords or not reusable.economics.haveCrystalsChanged():
        return None
    else:
        return _FinancialRecords(main=crystalsRecords, additional=additionalRecords, alternative=None)
...
def getTotalXPToShow(reusable, records, hasPremium):
    if hasPremium:
        return reusable.economics.getXPToShow()[0]
    return records.getRecord('xpToShow')
...
def getTotalCreditsToShow(reusable, records, hasPremium):
    if hasPremium:
        return reusable.economics.getCreditsToShow()[0]
...
def _createPremiumBonusModel(reusable):
    if reusable.economics.isPremiumPlus or reusable.economics.isActivePremiumPlus():
        return None
    else:
        return getCurrencies((reusable.economics.getCreditsDiff(), reusable.economics.getXPDiff()), (CurrencyModel.CREDITS, CurrencyModel.XP))