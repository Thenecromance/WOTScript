...
class TradeInController(ITradeInController):
    ...
    def getActiveTradeOffVehicle(self):
        if self.__activeTradeOffCD:
            return first(self.itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.IN_CD_LIST([self.__activeTradeOffCD])).values())
        else:
            return None
    ...
    def getActiveTradeOffVehicleState(self):
        ...
        if activeTradeOffVehicle is None:
            return (None, None)
        else:
            return activeTradeOffVehicle.getState()
    ...
    def getMinAcceptableSellPrice(self):
        if self.isEnabled():
            return self.__config.minAcceptableSellPrice
        return 0
