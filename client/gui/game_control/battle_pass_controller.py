...
class BattlePassController(IBattlePassController):
    ...
    def canChooseAnyReward(self):
        if not self.isOfferEnabled():
            return False
        return any((token.startswith(BATTLE_PASS_CHOICE_REWARD_OFFER_GIFT_TOKENS) for token in self.__itemsCache.items.tokens.getTokens().iterkeys() if self.__offersProvider.getOfferByToken(getOfferTokenByGift(token)) is not None))
    ...
    def getFullChapterPoints(self, chapter, includeCurrent):
        ...
        if minLevel <= 1:
            return 0
        return levelsConfig[minLevel - 2]
