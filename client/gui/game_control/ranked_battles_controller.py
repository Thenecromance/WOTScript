...
class RankedBattlesController(IRankedBattlesController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def _getAlertBlockData(self):
        if not self.hasSuitableVehicles():
            return self._ALERT_DATA_CLASS.constructForVehicle(self.getSuitableVehicleLevels(), self.vehicleIsAvailableForBuy(), self.vehicleIsAvailableForRestore())
    ...
    def __getQuestForRank(self, rankId):
        ...
        if season is None:
            ...
            return first(self.__eventsCache.getRankedQuests(lambda q: q.getRank() == rankId and q.isHidden() and q.isForRank() and q.getSeasonID() == season.getSeasonID()).values())
