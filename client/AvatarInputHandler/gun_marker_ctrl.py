...
class _CrosshairShotResults(object):
    ...
    def getShotResult(cls, hitPoint, collision, direction, excludeTeam = 0, piercingMultiplier = 1):
        ...
        if not isinstance(entity, (VehicleEntity, DestructibleEntity)):
            return _SHOT_RESULT.UNDEFINED
        elif entity.health <= 0 or entity.publicInfo['team'] == excludeTeam:
            return _SHOT_RESULT.UNDEFINED
        player = BigWorld.player()
        if player is None:
            return _SHOT_RESULT.UNDEFINED
        ...
        if shellKind == constants.SHELL_TYPES.HIGH_EXPLOSIVE and shell.type.mechanics == constants.SHELL_MECHANICS_TYPE.MODERN:
            return cls.__shotResultModernHE(collisionsDetails, fullPiercingPower, shell, minPP, maxPP, entity)
        else:
            return cls.__shotResultDefault(collisionsDetails, fullPiercingPower, shell, minPP, maxPP, entity)
