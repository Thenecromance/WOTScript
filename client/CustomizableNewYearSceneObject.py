...
class CustomizableNewYearSceneObject(NewYearSelectableObject):
    ...
    def _getLocalTransformMatrix(self, nodeName, modelName):
        ...
        if modelTransformation is not None and modelTransformation.transform:
            return modelTransformation.transform
        else:
            return

    def _getModelOverride(self, nodeName, modelName):
        modelTransformation = self._getModelTransformation(nodeName, modelName)
        if modelTransformation is not None and modelTransformation.modelOverride:
            return modelTransformation.modelOverride
        else:
            return

    def _getModelTransformation(self, nodeName, modelName):
        nodeKey = (self.anchorName, nodeName)
        if modelName in g_cache.toysTransformations and nodeKey in g_cache.toysTransformations[modelName]:
            return g_cache.toysTransformations[modelName][nodeKey]