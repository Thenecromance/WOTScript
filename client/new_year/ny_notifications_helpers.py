...
class LootBoxNotificationHelper(Notifiable):
    ...
    def prepareNotifications(self, tokens):
        for tokenID, data in tokens.iteritems():
            ...
            if lootBox is None or lootBox.getType() not in (NewYearLootBoxes.COMMON, NewYearLootBoxes.PREMIUM):
                continue
    ...
    def __timeTillNextNotification(self):
        if not self.__nyController.isEnabled():
            return 0.0
        if any([ value > 0 for value in self.__delayedNotifications.itervalues() ]):
            return _TIME_DELAY_CONSTANT
        return 0.0
