...
def _getPostEventMessage(ctrl):
    if ctrl.getVehicleBranch().hasAvailableSlots():
        return _NewYearSysMessages(R.strings.ny.notification.postEvent(), 'medium', SystemMessages.SM_TYPE.Information)
...
class NewYearController(INewYearController, IGlobalListener):
    ...
    def getActiveSettingBonusValue(self):
        if self.isPostEvent():
            return self._getPostEventBonus()
        return CreditsBonusHelper.getBonus()

    def getActiveMultiplier(self):
        if self.isPostEvent():
            return self._getPostEventAtmosphereMultiplier()
        return CreditsBonusHelper.getAtmosphereMultiplier()

    def getActiveMegaToysBonus(self):
        if self.isPostEvent():
            return self._getPostEventMegaToysBonus()
        return CreditsBonusHelper.getMegaToysBonus()

    def getActiveCollectionsBonus(self):
        if self.isPostEvent():
            return self._getPostEventCollectionsBonus()
        return CreditsBonusHelper.getCollectionsFactor()
    ...
    def getPureSlotForToy(self, toy):
        ...
        if fittingSlot is not None:
            return fittingSlot.id
        else:
            return
    ...
    def checkForNewToys(self, slot = None, objectType = None):
        ...
        def _atmospherePredicate(toyToCheck):
            ...
            return toyToCheck.getPureCount() > 0
    ...
    def getMaxToysStyle(self):
        ...
        if not toys:
            ...
            return max(ToySettings.NEW, key=lambda style: len([ toy for toy in toys if toy.setting == style ]))
