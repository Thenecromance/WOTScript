...
class VehicleSlot(object):
    ...
    def isAvailable(self):
        ...
        if restriction == NYVehBranchConsts.LEVEL:
            return self.__itemsCache.items.festivity.getMaxLevel() >= self.getLevel()
        return False
    ...
    def getRestrictionType(self):
        ...
        if NYVehBranchConsts.TOKEN in slotInfo:
            return NYVehBranchConsts.TOKEN
        return NYVehBranchConsts.UNDEFINED
    ...
    def getVehicle(self):
        ...
        if invID != EMPTY_VEH_INV_ID:
            return self.__itemsCache.items.getVehicle(invID)
        else:
            return None
...
class VehicleBranch(object):
    ...
    def hasSlotWithLevel(level):
        if NY_BRANCH_MIN_LEVEL <= level <= NY_BRANCH_MAX_LEVEL:
            return True
        return False
...
class SetVehicleBranchProcessor(Processor):
    ...
    def _successHandler(self, code, ctx = None):
        ...
        elif slot.getVehicle() is not None:
            return self.__makeSuccess(None, slot)
        else:
    ...
    def __getSysMsgKey(self, currency = None):
        if currency is not None:
            return '{}/{}/success'.format(self.__getMessagePrefix(), currency)
        else:
            return '{}/success'.format(self.__getMessagePrefix())

    def __getMessagePrefix(self):
        return 'newYear/setVehicleBranch'
...
class SetVehicleBranchSlotBonusProcessor(Processor):
    ...
    def _getMessagePrefix(self):
        return 'newYear/setVehicleBranchSlotBonus'
    ...
    def _successHandler(self, code, ctx = None):
        ...
        if bonus is not None:
            return makeNYSuccess(self._getSysMsgKey(bonus[0]), msgType=SM_TYPE.NewYearVehicleBranchBonus, slotName=_getSlotName(slot, case='gen'), bonus=str(int(bonus[1] * PERCENT)) + '%')
        else:
            return super(SetVehicleBranchSlotBonusProcessor, self)._successHandler(code, ctx)
