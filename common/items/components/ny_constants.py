...
class YEARS_INFO(object):
    ...
    def getCollectionTypesByYear(year, useMega = True):
        ...
        return filter(lambda x: x != ToySettings.MEGA_TOYS, YEARS_INFO._COLLECTION_TYPES_BY_YEAR[year])
...
class CelebrityQuestTokenParts(object):
    ...
    def sameQuestValidator(cls, quest1, quest2):
        ...
        if any([ cls.POSTFIX_SIMPLIFIED != postfix for postfix in questsPostfixes ]):
            return False
        return quest1.split(cls.SEPARATOR)[:2] == quest2.split(cls.SEPARATOR)[:2]
    ...
    def getQuestInfo(cls, questToken):
        ...
        if questLevel == CelebrityQuestLevels.NONE:
            return (None, None)
        questPrefix = parts[0]
        if not questPrefix.startswith(cls.PREFIX):
            return (None, None)
        else:
            return (questLevel, questPrefix)
    ...
    def getLevelFromSimplificationToken(cls, simplificationToken):
        ...
        if levelNum == CelebrityQuestLevels.NONE:
            return None
        else:
            return levelNum
    ...
    def generateSimplificationToken(cls, simplLevel, questPrefix):
        ...
        if levelName is None:
            ...
            return '{prefix}{separator}{levelName}{separator}{postfixSimpl}'.format(separator=cls.SEPARATOR, prefix=questPrefix, levelName=levelName, postfixSimpl=cls.POSTFIX_SIMPLIFIED)
    ...
    def getQuestNameForLevel(cls, questLevel, questPrefix):
        ...
        if levelName is None:
            ...
            return '{}{}{}'.format(questPrefix, cls.SEPARATOR, levelName)
