...
class Equipment(Artefact):
    __slots__ = ('equipmentType', 'reuseCount', 'cooldownSeconds', 'soundNotification', 'soundPressedReady', 'soundPressedNotReady', 'stunResistanceEffect', 'stunResistanceDuration', 'repeatedStunDurationFactor', 'consumeSeconds', 'deploySeconds', 'rechargeSeconds')
...
class ExtraHealthReserve(StaticOptionalDevice):
    ...
    def weightOnVehicle(self, vehicleDescr):
        ...
        level = self.defineActiveLevel(vehicleDescr)
        if level is None:
            return super(ExtraHealthReserve, self).weightOnVehicle(vehicleDescr)
        else:
            return (self._vehWeightFraction, self._weight, chassis.maxLoad * (self.chassisMaxLoadFactor.getActiveValue(level) - 1.0))
...
class AreaOfEffectEquipment(Equipment, TooltipConfigReader, SharedCooldownConsumableConfigReader, ArcadeEquipmentConfigReader):
    ...
    def _readEffectConfig(self, section):
        if not section:
            ...
            return {'shotEffects': section.readString('shotEffects').split(),
